...
class Llm:
    ...
    def find_parameters(
        ...
        authorized_tokens = self.find_tokens(prompt_token, kind, example)
    ...
    def find_tokens(self, prompt: list[int], kind: str, example: dict) -> list[int]:
        '''Recuperate authorized tokens for parameter'''
        authorized: list = prompt
        # authorized.extend(self.llm.encode(json.dumps(example))[0].tolist())
        if kind == "number":
            numb = [i for i in range(15, 25)]
            numb.extend([13])
            authorized = [i for i in authorized if i in numb]
            if not authorized:
                raise(Exception("[ERROR]: The prompt is not precise enough"))
        for key in example.keys():
            print(key)
            authorized.extend(self.llm.encode(key)[0].tolist())
        authorized.extend([4913, 9207, 788, 330, 151645])
        return authorized
    ...
    def p_str_answer_building(self, sentence: list[int], restreigned_code: list[int]) -> list[int]:
        ...
        while token_last_word != 151645 and token < 150:
            ...
            token_last_word = next_token
            token += 1
        self.see_tokens(list(logits))
        return (sentence[start:])

    def see_tokens(self, liste: list[float]) -> None:
        trad = []
        for i in liste:
            if i == -np.inf:
                continue
            trad.append(self.llm.decode([liste.index(i)]))
        print(trad)
